<x-app-layout>
    <div class="py-12" x-data="eventManager({
        initialEvents: @js($events->items()),
        initialPagination: @js([
        ...
        initialCategoryId: @js(request('category', '')),
        csrf: @js(csrf_token()),
        deleteConfirmMessage: @js(__('messages.confirm_delete_event')),
        createSuccessMessage: @js(__('messages.event_created')),
        updateSuccessMessage: @js(__('messages.event_updated')),
        deleteSuccessMessage: @js(__('messages.event_deleted'))
    })" x-init="init()">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Table Wrapper -->
            <div id="table-wrapper" x-show="!loading">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="relative transform overflow-hidden rounded-xl bg-white text-left shadow-2xl transition-all w-full max-w-2xl border border-slate-200">
                ...
                <div class="bg-white px-6 py-4 border-b border-slate-200 flex items-center justify-between">
                    <h3 class="text-xl font-bold text-slate-900" id="modal-title" x-text="isEdit ? 'Edit Event' : 'Create New Event'"></h3>
                ...
                <form @submit.prevent="submitForm()" class="p-6 space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" x-model="formData.title" id="title" class="block w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-slate-900 placeholder:text-slate-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" placeholder="e.g., Annual Tech Conference">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="datetime-local" name="event_date" x-model="formData.event_date" id="event_date" class="block w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">
                            <template x-if="errors.event_date">
                                <p class="mt-1 text-sm text-red-600" x-text="errors.event_date[0]"></p>
                            </template>
                        </div>
                        <div>
                            <label for="status" class="block text-sm font-semibold text-slate-700 mb-2">Status</label>
                            <div class="relative">
                                <select name="status" x-model="formData.status" id="status" class="appearance-none block w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 pr-10 text-slate-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            @foreach($categories as $category)
                                <div class="relative">
                                    <input type="checkbox" name="categories[]" x-model="formData.categories" value="{{ $category->id }}" id="cat_{{ $category->id }}" class="peer sr-only">
                        ...
                        <template x-if="errors.categories">
                            <p class="mt-1 text-sm text-red-600" x-text="errors.categories[0]"></p>
                        </template>
                    </div>
                    ...
                    <div>
                        ...
                        <div x-show="imagePreview" class="mb-3">
                            <img :src="imagePreview" alt="Preview" class="max-w-full h-48 object-cover rounded-lg border border-slate-200">
                        </div>
                        ...
                        <div class="flex items-center justify-center w-full">
                            <label for="image" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-300 rounded-lg cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors">
                                ...
                                <input type="file" name="image" id="image" @change="handleFileUpload($event)" class="hidden" accept="image/*">
                            </label>
                        </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
                        ...
                        <button type="submit" :disabled="loading" class="px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                            <span x-show="!loading" x-text="isEdit ? 'Update Event' : 'Create Event'"></span>
